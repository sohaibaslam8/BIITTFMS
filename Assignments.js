...
class Assignments extends Component {
  ...
  MoveLeftFunction() {
    var week = lib.WeekNoAssignments.split('-')[0]
    var weekno = lib.WeekNoAssignments.split('-')[1]
    if (weekno > 1) {
      weekno--;
      lib.WeekNoAssignments = `${week}-${weekno}`
      this.componentDidMount();
  ...
  componentDidMount() {
    ...
    const url = `http://192.168.43.143/FWebAPI/api/users/AllDocumentShowSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoAssignments}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
  ...
  addFolderDetail() {
    ...
    collection.Doc_Type = 'Assignment';
  ...
  GetFolderDetailId() {
    const url = `http://192.168.43.143/FWebAPI/api/Users/GetFolderDetailIdSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoAssignments}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
