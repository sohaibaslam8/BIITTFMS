...
class QuizzesSolution extends Component {
  constructor(props) {
    super(props);
  ...
  HideModel(week) {
    lib.WeekNoQuizzesSolutions = week
  ...
  MoveLeftFunction() {
    var week = lib.WeekNoQuizzesSolutions.split('-')[0]
    var weekno = lib.WeekNoQuizzesSolutions.split('-')[1]
    if (weekno > 1) {
      weekno--;
      lib.WeekNoQuizzesSolutions = `${week}-${weekno}`
      this.componentDidMount();
  ...
  componentDidMount() {
    ...
    const url = `http://192.168.43.143/FWebAPI/api/users/AllDocumentShowSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoQuizzesSolutions}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
  ...
  addFolderDetail() {
    ...
    collection.Doc_Type = 'QuizSolution';
    collection.Doc_Status = lib.WeekNoQuizzesSolutions;
  ...
  GetFolderDetailId() {
    const url = `http://192.168.43.143/FWebAPI/api/Users/GetFolderDetailIdSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoQuizzesSolutions}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
      ...
      this.state.isloading
        ...
        <View style={styles.maincontainer}>
          ...
          <View style={{ marginLeft: '70%', flexDirection: 'row', margin: 20 }}>
            ...
            {/* } */}
            {/* {lib.WeekNoQuizzesSolutions != 'Week-16' && */}
...
export default QuizzesSolution;
