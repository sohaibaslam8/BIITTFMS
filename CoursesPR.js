...
import { Text, View, FlatList, StyleSheet, ActivityIndicator, TouchableOpacity, Image, Keyboard,Linking, TouchableWithoutFeedback } from 'react-native';
...
export default class CoursesPW extends React.Component {
    ...
    ShowFiles = () => {
        ...
        Linking.canOpenURL(`http://192.168.10.3/FWebAPI/`+`${lib.FileNamePR}`).then(supported => {
          if (supported) {
            Linking.openURL(`http://192.168.10.3/FWebAPI/`+`${lib.FileNamePR}`);
          } else {
            console.log("Don't know how to open URI: " + `http://192.168.10.3/FWebAPI/`+`${lib.FileNamePR}`);
          }
        });
      };
    getFile(cno) {
    ...
    ShowNewScreenC(cno, cname) {
        ...
        lib.CNamePR = cname;
        this.getFile(cno);
        this.props.navigation.navigate('ShowFilePR');
    }
    renderItemC = ({ item }) => {
        return (
            <TouchableOpacity
