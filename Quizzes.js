...
class Quizzes extends Component {
  ...
  MoveLeftFunction() {
    var week = lib.WeekNoQuizzes.split('-')[0]
    var weekno = lib.WeekNoQuizzes.split('-')[1]
    if (weekno > 1) {
      weekno--;
      lib.WeekNoQuizzes = `${week}-${weekno}`
      this.componentDidMount();
  ...
  componentDidMount() {
    ...
    const url = `http://192.168.10.5/FWebAPI/api/users/AllDocumentShowSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoQuizzes}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
  ...
  GetFolderDetailId() {
    const url = `http://192.168.10.5/FWebAPI/api/Users/GetFolderDetailIdSubFolderQuizAndAssignment?courseno=${lib.CNo}&section=${lib.Section}&discipline=${lib.Discipline}&semc=${lib.Semc}&semno=${lib.SemNoTemp}&empno=${lib.TIdTemp}&dtype=${this.state.dtype}&dstatus=${lib.WeekNoQuizzes}`
    fetch(url)
      .then((response) => response.json())
      .then((responsejson) => {
        console.log(responsejson)
